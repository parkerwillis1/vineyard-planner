-- Bottling Management Tables (FIXED)
-- References production_lots instead of wine_lots
...
CREATE TABLE IF NOT EXISTS bottled_inventory (
  ...
  -- TTB fields (added for compliance)
  bond_status TEXT DEFAULT 'in_bond' CHECK (bond_status IN ('in_bond', 'taxpaid', 'exported', 'transferred')),
  ttb_tax_class TEXT,
  gallons_equivalent DECIMAL(12, 4),
...
CREATE POLICY "Users can delete their own bottled inventory"
  ON bottled_inventory FOR DELETE
  USING (auth.uid() = user_id);

-- RLS Policies for QC checks
...
-- Add FK constraint from ttb_transactions to bottling_runs now that it exists
DO $$
BEGIN
  IF NOT EXISTS (
...
EXCEPTION WHEN OTHERS THEN
  -- Ignore if ttb_transactions doesn't exist yet
  NULL;
END $$;
